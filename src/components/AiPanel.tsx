...
const ALLOWED: BlockType[] = [
  "heading1", "heading2", "heading3", "paragraph", "code", "table", "callout",
    ...
    <Modal open={open} title="AI Generate" onClose={onClose}>
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-end gap-2">
          <button type="button" className="rounded-md px-3 py-2 text-sm text-zinc-600 hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-900" onClick={onClose}>Cancel</button>
          <Button onClick={run} disabled={busy}>{busy ? "Generating…" : "Generate"}</Button>
        </div>
        <div className="text-xs text-zinc-500 dark:text-zinc-400">
          Requires the OpenDocs server running (NVIDIA proxy). Set <code className="rounded bg-zinc-100 px-1 dark:bg-zinc-900">API_AUTH_TOKEN</code> + <code className="rounded bg-zinc-100 px-1 dark:bg-zinc-900">VITE_API_AUTH_TOKEN</code> for production.
        </div>
      </div>
    </Modal>
  );
