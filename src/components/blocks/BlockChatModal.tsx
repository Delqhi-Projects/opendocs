...
import { Wand2, Zap, AlignLeft, ShieldCheck, Languages } from "lucide-react";

type Msg = { role: "user" | "assistant"; content: string; commands?: OpenDocsCommand[] };

type CommandRowProps = { cmd: OpenDocsCommand };
...
export function BlockChatModal({
  ...
  const context = useMemo(() => summarizeBlock(block), [block]);
  ...
  return (
    <Modal open={open} title={`Block Chat • ${currentBlock.type}`} onClose={onClose}>
