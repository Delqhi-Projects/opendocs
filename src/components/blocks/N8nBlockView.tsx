...
import type { N8nNodeDefinition } from "@/services/n8n";
...
export function N8nBlockView({
  ...
  const saveNode = async () => {
    ...
    if (!data.workflowId) {
      ...
      onUpdate({ data: { ...data, workflowId: wf.id } } as any);
    }
    ...
    onUpdate({ data: { ...data, workflowId: data.workflowId || "", node: { ...data.node, name: nodeRes.nodeId } } } as any);
  };
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={async () => {
              ...
              onUpdate({ data: { ...data, node: { ...data.node, disabled: next } } } as any);
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="rounded-lg border border-zinc-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-950">
          ...
          <select
            ...
            onChange={(e) => onUpdate({ data: { ...data, node: { ...data.node, nodeType: e.target.value } } } as any)}
          ...
          <textarea
            ...
            onChange={(e) => {
              try {
                ...
                onUpdate({ data: { ...data, node: { ...data.node, parameters: parsed } } } as any);
              } catch {
          ...
          <div className="space-y-1">
             ...
             <div className="flex flex-wrap gap-1.5 p-1 rounded-md border border-zinc-100 dark:border-zinc-900 bg-zinc-50/50 dark:bg-zinc-900/30">
               {(state.pages?.[state.selectedPageId || ""]?.blocks || [])
                 .filter(b => b?.type === "n8n" && b?.id !== block?.id)
                 .map((other: any) => {
                   ...
                   return (
                     <button
                       ...
                       onClick={async () => {
                         ...
                         onUpdate({ data: { ...data, connections: next } } as any);
                       }}
                       className={cn(
               ...
               {(state.pages?.[state.selectedPageId || ""]?.blocks || []).filter(b => b?.type === "n8n" && b?.id !== block.id).length === 0 && (
